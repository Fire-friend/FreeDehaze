...
def conv_forward2(self):
    ...
    def forward(input_tensor, temb, scale=1.0):
        ...
        if self.injection_schedule is not None and (
            ...
            source_hidden = hidden_states[2 * source_batch_size : 3 * source_batch_size]
            hidden_states[0 * source_batch_size : 1 * source_batch_size] = source_hidden
            hidden_states[1 * source_batch_size : 2 * source_batch_size] = source_hidden
            hidden_states[3 * source_batch_size : 4 * source_batch_size] = source_hidden
            hidden_states[4 * source_batch_size : 5 * source_batch_size] = source_hidden
        if self.conv_shortcut is not None:
            input_tensor = self.conv_shortcut(input_tensor)
        output_tensor = (input_tensor + hidden_states) / self.output_scale_factor
        return output_tensor
...
def prep_unet_conv(unet, injection_schedule=None):
    for i in range(len(unet.up_blocks)):
        for j in range(len(unet.up_blocks[i].resnets)):
            module = unet.up_blocks[i].resnets[j]
    ...
    if injection_schedule is not None:
        ...
        conv_module.forward = conv_forward2(conv_module)
